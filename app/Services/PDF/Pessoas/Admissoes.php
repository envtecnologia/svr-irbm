...
class Admissoes extends PdfService
{
    /**
    ...
    function admissoesPdf($dados)
    {
        ...
        if ($dados->isNotEmpty()) {
            ...
            $this->SetX(22);
            $this->SetFont("Arial", "B", 10);
            $this->Cell(250, 6, iconv("utf-8","iso-8859-1", "Relatórios de Admissões ({$dados->count()} registros)"), 1, 0, "L", TRUE);
            $this->Ln();
            $this->Ln(2);
            ...
            foreach ($dados as $admissao)
            {
                $dataSaida = substr($admissao["datacadastro"], 8, 2) . "/"
                           . substr($admissao["datacadastro"], 5, 2) . "/"
                           . substr($admissao["datacadastro"], 0, 4);

                if ($provincia != $admissao["provincia"]["descricao"])
                {
                    $this->SetDrawColor(220);
                    $this->SetFillColor(196,210,205);
                    ...
                    $this->SetX(22);
                    $this->SetFont("Arial", "B", 8);
                    $this->Cell(250, 6, iconv("utf-8","iso-8859-1", $admissao["provincia"]["descricao"]), 1, 0, "L", TRUE);
                    $this->Ln();

                    $this->SetFillColor(204);
                    $provincia = $admissao["provincia"]["descricao"];

                    $this->SetX(22);
                    $this->SetFont("Arial", "B", 8);
                    $this->Cell(28, 6, iconv("utf-8","iso-8859-1", "Data Cadastro"), 1, 0, "C", TRUE);
                    $this->Cell(112, 6, iconv("utf-8","iso-8859-1", "Pessoa"), 1, 0, "L", TRUE);
                ...
                $this->Cell(112, 6, iconv("utf-8","iso-8859-1", "{$admissao["sobrenome"]}, {$admissao["nome"]}"), 1, 0, "L", FALSE);
                $telefone1 = ($admissao["telefone1"] != "" ? $admissao["telefone1"] : "---");
                $this->Cell(30, 6, iconv("utf-8","iso-8859-1", $telefone1), 1, 0, "C", FALSE);
                $email = ($admissao["email"] != "" ? $admissao["email"] : "---");
