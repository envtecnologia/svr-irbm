...
class Dioceses extends PdfService
{
    /**
    ...
    function diocesesPdf($dados)
    {
        ...
        if ($dados->isNotEmpty()) {
            ...
            $this->SetX(22);
            $this->SetFont("Arial", "B", 10);
            $this->Cell(250, 6, iconv("utf-8", "iso-8859-1", "Relatório de Dioceses ({$dados->count()} registros)"), 1, 0, "L", TRUE);
            $this->Ln();
            $this->Ln(2);
            ...
            $this->SetX(22);
            $this->SetFont("Arial", "B", 8);
            $this->Cell(70, 6, iconv("utf-8", "iso-8859-1", "Diocese"), 1, 0, "L", TRUE);
            ...
            $this->Cell(50, 6, iconv("utf-8", "iso-8859-1", "Bispo"), 1, 0, "L", TRUE);
            $this->SetTextColor(0);
            $this->Ln();

            $quantitativo = 0;

            foreach ($dados as $diocese) {
                if ($diocese["situacao"] == "1") {
                    $encerramento = "Ativa";
                } else {
                    $encerramento = "Encerrada (" .
                        substr($diocese["encerramento"], 8, 2) . "/" .
                        substr($diocese["encerramento"], 5, 2) . "/" .
                        substr($diocese["encerramento"], 0, 4) . ")";
                }

                $cidade = ($diocese["cidade"] != "" ? $diocese["cidade"]->descricao : "---");
                $telefone1 = ($diocese["telefone1"] != "" ? $diocese["telefone1"] : "---");
                $email = ($diocese["email"] != "" ? $diocese["email"] : "---");
                $contato = ($diocese["bispo"] != "" ? $diocese["bispo"] : "---");
                ...
                $this->Cell(62, 6, iconv("utf-8", "iso-8859-1", $diocese["descricao"]), 1, 0, "L", FALSE);
                ...
                $this->Cell(50, 6, iconv("utf-8", "iso-8859-1//TRANSLIT", $contato), 1, 0, "L", FALSE);
                $this->SetTextColor(0);
                $this->Ln();
            }
